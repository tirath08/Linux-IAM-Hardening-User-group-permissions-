...
Developers - Application-specific commands only %dev ALL=(root) /usr/bin/systemctl restart nginx, /usr/bin/systemctl status nginx
...
sudo chmod -R u=rwx,g=rwx,o= /var/log/myapp
...
sudo touch /etc/passwd.backup
sudo touch /etc/shadow.backup
sudo chmod 644 /etc/passwd.backup /etc/shadow.backup
